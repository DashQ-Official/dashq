...
const PG_URL = process.env.DASHQ_TEST_POSTGRES_URL;

describe.skipIf(!PG_URL)("createPostgresAdapter", () => {
  let adapter: DatabaseAdapter;
  let testClient: import("pg").Client;

  beforeAll(async () => {
    adapter = createPostgresAdapter(PG_URL!);
    await adapter.initialize();

    // Separate client for test infrastructure SQL (truncate, drop, etc.)
    const { default: pg } = await import("pg");
    testClient = new pg.Client({ connectionString: PG_URL! });
    await testClient.connect();
  });

  beforeEach(async () => {
    await testClient.query(
      "TRUNCATE dashq_jobs, dashq_job_logs CASCADE",
    );
  });

  afterAll(async () => {
    await testClient.query(
      "DROP TABLE IF EXISTS dashq_job_logs, dashq_jobs, dashq_meta CASCADE",
    );
  ...
  describe("initialize / close", () => {
    ...
    it("is idempotent (calling initialize twice doesn't error)", async () => {
      // Create a second adapter on the same DB — its initialize should not throw
      const adapter2 = createPostgresAdapter(PG_URL!);
      await expect(adapter2.initialize()).resolves.not.toThrow();
  ...
  describe("getJob", () => {
    ...
    it("returns null for non-existent id", async () => {
      const fetched = await adapter.getJob(
  ...
  describe("listJobs", () => {
    ...
    it("combines status + job_type filters", async () => {
      const j1 = await adapter.insertJob(
        createJob({ job_type: "email.send" }),
      );
      await adapter.insertJob(createJob({ job_type: "email.send" }));
      await adapter.insertJob(createJob({ job_type: "report.generate" }));
  ...
  describe("deleteOldJobs", () => {
    it("deletes jobs older than the given date in specified statuses", async () => {
      ...
      // Use a boundary in the future so the job qualifies
    ...
    it("does not delete jobs in non-specified statuses", async () => {
      ...
      const boundary = new Date(Date.now() + 1_000);
      const count = await adapter.deleteOldJobs(boundary, [
        "succeeded",
        "failed",
      ]);
      expect(count).toBe(0);
  ...
  // -----------------------------------------------------------------------
  // Postgres-specific: concurrent claimNextJob
  // -----------------------------------------------------------------------

  describe("concurrent claimNextJob", () => {
    it("1 job + 2 concurrent claimers → exactly 1 wins", async () => {
      await adapter.insertJob(createJob());

      // Create a second adapter to simulate a second worker
      const adapter2 = createPostgresAdapter(PG_URL!);
      await adapter2.initialize();

      const worker1 = { worker_id: "w-1", lease_duration_ms: 30_000 };
      const worker2 = { worker_id: "w-2", lease_duration_ms: 30_000 };

      const [claim1, claim2] = await Promise.all([
        adapter.claimNextJob(worker1),
        adapter2.claimNextJob(worker2),
      ]);
    ...
    it("2 jobs + 2 concurrent claimers → each gets a different job", async () => {
      const jobA = await adapter.insertJob(createJob());
      const jobB = await adapter.insertJob(createJob());
      ...
      // Both original job IDs should be covered
      const claimedIds = new Set([claim1!.id, claim2!.id]);
      expect(claimedIds.has(jobA.id) || claimedIds.has(jobB.id)).toBe(true);
